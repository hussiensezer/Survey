...
class GeneralController extends Controller implements SurveyInterface
{
    public SurveyPattern $surveyPattern;
    public const  SURVEY_TYPE = 'general';

    public function __construct(SurveyPattern $surveyPattern)
    {
        $this->surveyPattern = $surveyPattern;
        $this->middleware('permission:general_add', ['only' => ['store']]);
        $this->middleware('permission:survey_edit', ['only' =>   ['update', 'edit']]);
    }// End Construct

    public function show(Survey $survey)
    {
        return view('dashboard.surveys.general.show', compact('survey'));
    }// End Show

    public function create()
    {
        $data = $this->surveyPattern->surveyType(self::SURVEY_TYPE)->create();
        return view('dashboard.surveys.general.create')->with($data);
    }// End Create
    ...
    public function edit(Survey $survey)
    {
        ...
        return view('dashboard.surveys.general.edit')->with($data);
    }// End Edit
